...
my $outDir = "data";
my $seqTrackName = "DNA";
my $seqTrackType = "WebApollo/View/Track/AnnotSequenceTrack";
my $seqTrackStore = "WebApollo/Store/SeqFeature/ScratchPad";
my $seqTrackUrlTemplate = "sequence_alterations_trackData.json";
my $help;
my $compress;
...
if (defined($gff)) {
    ...
    if ($#refSeqs < 0) {
		process_fasta(($tmpFasta->filename));
	}
                                    ...
                                    $tracks->[$i] =
                                      {
                                       'label' => $seqTrackName,
                                       'type' => $seqTrackType,
                                       'storeClass' => $seqTrackStore,
                                       'key' => $seqTrackName,
				       'config' => {
                                         'chunkSize' => $chunkSize,
					 'residuesUrlTemplate' => "$seqRel/{refseq}/",
                                         'urlTemplate' => $seqTrackUrlTemplate,
					 'style' => {
						 'uniqueIdField' => 'id',
						 'className' => '{type}'
					 },
