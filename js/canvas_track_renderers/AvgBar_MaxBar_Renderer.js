...
function AvgBar_MaxBar_Renderer()  {  }

AvgBar_MaxBar_Renderer.NAME = "AvgBar + MaxBar";
AvgBar_MaxBar_Renderer.ID = "AvgBar_MaxBar";

CanvasTrack.registerRenderer(AvgBar_MaxBar_Renderer);

AvgBar_MaxBar_Renderer.drawBlock = function (track, blockIndex, blockDiv,
					     leftBlock, rightBlock,
					     leftBase, rightBase,
					     scale, stripeWidth,
					     containerStart, containerEnd) {
    if (CanvasTrack.VERBOSE_RENDER) {  console.log("called AvgBar_MaxBar_Renderer.drawBlock");  }
...
//	console.log(level);
    ...
    // var cheight = (track.maxviz - track.minviz); 
    // canvas.height = 100; 
    var cheight = 100;
...
//    context.translate(-leftBase, 0);
	...
	for (var index = mindex; index <= maxdex; index++)  {
		...
		context.rect(coord, cheight, 1, (score * -cheight));
	    }
	    else  {
		// context.fillRect(coord, 100, 1, (score * -100));
		context.fillRect(coord, cheight, 1, (score * -cheight));
	    }
	}
    }
	   ...
	   //  console.log("score: " + coords[mindex]);
	    ...
	    for (var index = mindex; index <= maxdex; index++) {
		 ...
		 if (coord == undefined || max == undefined)  {
		     ...
		     context.fillRect(coord, cheight, bases_per_pixel, (max * -cheight));
...
//		    context.fillRect(coord, 100 - (max * 100), 1 * bases_per_pixel, 1);
		...
		if (coord == undefined || avg == undefined)  {  // skip edge cases where avg is undefined...
		    ...
		    context.fillRect(coord, cheight, bases_per_pixel, (avg * -cheight));
		}
	    }
...
*/
		...
		// if (score)  {  // skip edge cases where score is undefined...
			...
			context.rect(coord, cheight, bases_per_pixel, (score * -cheight));
		    }
		    else  {
			// context.fillRect(coord, 100, bases_per_pixel, (score * -100));
			context.fillRect(coord, cheight, bases_per_pixel, (score * -cheight));
		    }
		//   }
	    }
	}
