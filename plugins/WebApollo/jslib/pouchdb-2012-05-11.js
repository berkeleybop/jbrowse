    ...
    xhr.onreadystatechange = function() {
      ...
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        call(onSuccess, data, xhr, callback);
          ...
          console.log("XHR NON-OK STATUS, status code: " + xhr.status)
         call(onError, xhr, callback);
      }
    };
...
function replicate(src, target, opts, promise) {
 ...
 /*   if (opts.heartbeat) {
        console.log("SETTING UP HEARTBEAT: " + opts.heartbeat);
        repOpts.heartbeat = opts.heartbeat;
    }  
*/                    
...
var HttpPouch = function(opts, callback) {
 ...
 /*   if (opts.heartbeat) {
        console.log("changes SETTING UP HEARTBEAT: " + opts.heartbeat);
