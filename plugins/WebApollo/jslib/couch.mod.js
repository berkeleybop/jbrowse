...
function CouchDB(name, httpHeaders) {
  this.name = name;
  // GAH was hardwired for same origin as page mod.couch.js script is loaded into
  //  changed so could be different origin (assuming CouchDB is configured for CORS)
  //  now requires that name arg be full URI to CouchDB database:  http://[couchDB_root]/[database_name]
...
CouchDB.allDbs = function(couchRootUri) {
  CouchDB.last_req = CouchDB.request("GET", (couchRootUri ? couchRootUri : "") + "/_all_dbs");
  CouchDB.maybeThrowError(CouchDB.last_req);
  return JSON.parse(CouchDB.last_req.responseText);
};
